...
if gum confirm "Update Chrome theme? If yes, Chrome (and all Chrome-based apps) will closed!"; then
	...
	jq ".ntp += {\"custom_background_dict\":{\"background_url\": \"https://github.com/Blinkuu/omacub/blob/master/themes/$CHROME_BACKGROUND?raw=true\"}}" Preferences >tmp.json && mv tmp.json Preferences
	cd ~
fi
